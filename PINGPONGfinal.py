...
jugadorA.color(Color1.get().upper())
jugadorA.penup()  # borra linea de huella al mover objetos
jugadorA.goto(-350, 0)
# dar fomma multiplicar largo y ancho
...
jugadorB.color(Color2.get().upper())
jugadorB.penup()
...
pen.goto(0, 260)
pen.write("Jugador A: 0               jugadorB: 0",
...
def guardarArchivo():
    ...
    for item in listaJugadores:
        linea = (item["Nombre1"] + ";" + item["P1"] +
                ";" + item["P2"] + ";" + item["Nombre2"])
        linea = linea.strip()
...
def opciones_Juego():
    ...
    if is_paused == True:
        ...
        buttonReanudar = tkinter.Button(
            ventana2, text="Reanudar", command=reanudar_Juego)
        buttonguardarpar = tkinter.Button(
            ventana2, text="Guardar Partida", command=guardarPartida)
        buttonlistarpar = tkinter.Button(
            ventana2, text="Listar Partidas", command=listarPartidas)
        buttonTerminar = tkinter.Button(
            ventana2, text="Terminar", command=terminar_Juego)
...
# cuando presiones la tecla w llama funcion subir
...
while True:
    if not is_paused:
        ...
        if pelota.xcor() > 390:
            ...
            pen.write(f"{Nombre1.get()}: {marcadorA}		{Nombre2.get()}: {marcadorB}",
                    align="center", font=("Courier", 25, "normal"))
